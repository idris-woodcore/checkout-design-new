              ...
              expireSignSubscribe.bind(event_name, (data) => {
                if (data?.status === true) {
                  window.location.replace(
                    `${successData?.merchantConfig?.redirect_link}?transaction_reference=${successData?.merchantConfig?.transaction_reference}&pooler_reference=${successData?.collection_data?.data?.reference}`
                  );
                  removeSubscription();
                  clearInterval(fetchDataInterval);
